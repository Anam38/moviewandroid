...
export class MoviewdetailPage implements OnInit {
  ...
  onScroll(event) {
    if(event.detail.scrollTop > 0){
      document.getElementById('header-detail').setAttribute("style", "background-color:#151517");
    }else if(event.detail.scrollTop  == 0 ){
      document.getElementById('header-detail').removeAttribute("style");
    }
  };
  ...
  async Recomendation(actor,title){
  ...
  detailmovie(movieId){
    ...
    console.log(this.dom.bypassSecurityTrustResourceUrl(url));
  ...
  async playvideo(url){
    ...
    setTimeout(function(){
      ...
      document.getElementById('header').setAttribute("style", "background-color: #151517");
      
      document.getElementById('frame-video').setAttribute("src",url);
      
      document.getElementById('movie-cards').setAttribute("style", "display:none;");
